...
func (r *CategoryRepository) InsertCategory(category model.Category) error {
	_, err := r.Conn.Exec(context.Background(),
		`INSERT INTO "category" (id, user_id, name, type, budget, created_at) VALUES ($1, $2, $3, $4, $5, $6)`,
...
func (r *CategoryRepository) GetAllCategories(userID uuid.UUID) ([]model.Category, error) {
	rows, err := r.Conn.Query(context.Background(),
		`SELECT id, user_id, name, type, budget, created_at FROM "category" WHERE user_id = $1`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *CategoryRepository) GetCategoryByID(id uuid.UUID) (model.Category, error) {
	...
	err := r.Conn.QueryRow(context.Background(),
		`SELECT id, user_id, name, type, budget, created_at FROM "category" WHERE id = $1`, id).
...
func (r *CategoryRepository) UpdateCategory(id uuid.UUID, category model.Category) error {
	_, err := r.Conn.Exec(context.Background(),
		`UPDATE "category" SET name=$1, type=$2, budget=$3 WHERE id=$4`,
		category.Name, category.Type, category.Budget, id)
	return err
}

func (r *CategoryRepository) DeleteCategory(id uuid.UUID) error {
	_, err := r.Conn.Exec(context.Background(), `DELETE FROM "category" WHERE id=$1`, id)
	return err
}
